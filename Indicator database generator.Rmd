...
```{r}
Fishcatalog<-readxl::read_excel("D:/Dropbox/Tesis Carlos Veloy/Scripts Carlos/Data/SppCatalogoWESTMEDcorrectspp.xlsx")%>%
...
Fish_n_Traits<- dplyr::filter (Fish_n_Traits,number != 0,FlagExclusionTax == 0)
```
...
for(t in c("All","PECES","MOLUSCOS","CRUSTÁCEOS")){
  ...
  for(P in c(0,1,2)){
    print(P)
   {
      ...
      write.csv(Indicators_table,(paste("Indicators_Tableregs_WestMed_",tax,pel,"points","_","Alldepths",".csv",sep="")))
    }}}
...
setwd(sectordatdir)
xlist<-list.files(pattern = "*.csv")
dbBase<-read.csv(paste(sectordatdir,"Indicators_Tableregs_WestMed_Allgamma_Alldepths.csv",sep="/"))
dbBase<-as.data.frame(dbBase[0,])
for(i in xlist) { 
  base <- read.csv(i,dec =".",sep=",")
  if ("X" %in% colnames(base)==TRUE){base<-dplyr::select(base,-(X))}
